...
namespace EmployeeManagerEngine.Data.Command.Mappings
{
    internal class EmployeeMapping : IEntityTypeConfiguration<Employee>
    {
        public void Configure(EntityTypeBuilder<Employee> builder)
        {
            ...
            builder.Property(p => p.Email).HasColumnName("email").HasMaxLength(100).HasColumnType("varchar").IsRequired();
            builder.Property(p => p.GenderId).HasColumnName("email").HasMaxLength(100).HasColumnType("varchar").IsRequired();
            //builder.Property(p => p.SkillIds).HasColumnName("email").HasMaxLength(100).HasColumnType("varchar").IsRequired();

            builder.HasOne(p => p.Gender).WithMany(p => p.Employees).HasForeignKey(fk => fk.GenderId).IsRequired();
            builder.HasMany(p => p.Skills).WithMany(p => p.Employees)
